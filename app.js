...
function handlePipelineEvent(data) {
  if (data.type === 'stage_start') setStageRunning(data.stage);
  if (data.type === 'stage_done') setStagesDone(data.stage);
}
...
function resetPipeline() {
  ['serp', 'gap', 'outline', 'draft', 'publish'].forEach(s => {
...
function renderQueueItem(job) {
  return `
    <div class="queue-item" id="qi-${job.id}">
      ...
      <div class="queue-keyword">${job.keyword}</div>
      <div class="queue-stage" id="qs-${job.id}">Pending</div>
      <div class="queue-time" id="qt-${job.id}"></div>
    </div>`;
}
...
function updateQueueItem(id, status, stageText) {
  ...
  if (!item) return;
  const dot = item.querySelector('.queue-status');
  dot.className = 'queue-status ' + status;
  document.getElementById('qs-' + id).textContent = stageText;
  if (status === 'done') {
    document.getElementById('qt-' + id).textContent = new Date().toLocaleTimeString();
  }
}
...
async function testConnections() {
  ...
  for (const svc of services) {
    results.insertAdjacentHTML('beforeend',
      `<div class="test-result-item checking" id="tr-${svc.key}">⏳ Testing ${svc.name}…</div>`);
    try {
      const resp = await fetch(`${API_BASE}/api/test/${svc.key}`);
      const data = await resp.json();
      document.getElementById('tr-' + svc.key).className = 'test-result-item ' + (data.ok ? 'ok' : 'fail');
      document.getElementById('tr-' + svc.key).textContent = (data.ok ? '✓ ' : '✗ ') + svc.name + ' — ' + data.message;
    } catch {
      document.getElementById('tr-' + svc.key).className = 'test-result-item fail';
      document.getElementById('tr-' + svc.key).textContent = '✗ ' + svc.name + ' — Could not reach backend';
    }
  }
}
